...
function switchSection(section) {
    ...
    var isMountain = (section === 'mountain');
    var isFacilities = (section === 'facilities');

    document.getElementById('roomsSection').style.display = isRooms ? 'block' : 'none';
    document.getElementById('mountainSection').style.display = isMountain ? 'block' : 'none';
    document.getElementById('facilitiesSection').style.display = isFacilities ? 'block' : 'none';

    document.getElementById('roomsTab').classList.toggle('active', isRooms);
    document.getElementById('mountainTab').classList.toggle('active', isMountain);
    document.getElementById('facilitiesTab').classList.toggle('active', isFacilities);

    // Show/hide category filters and add button based on section
    ...
    if (isRooms) {
        ...
        addBtn.setAttribute('data-bs-target', '#addRoomModal');
        addBtn.innerHTML = '<i class="bi bi-plus-lg"></i> <span>Add Room or Cottage</span>';
        showRooms();
    } else if (isMountain) {
        categoryFilters.style.display = 'none';
        addBtn.style.display = 'flex';
        addBtn.setAttribute('data-bs-target', '#addMountainModal');
        addBtn.innerHTML = '<i class="bi bi-plus-lg"></i> <span>Add Mountain View</span>';
        showMountainViews();
    } else if (isFacilities) {
        categoryFilters.style.display = 'none';
        addBtn.style.display = 'flex';
        addBtn.setAttribute('data-bs-target', '#addFacilityModal');
        addBtn.innerHTML = '<i class="bi bi-plus-lg"></i> <span>Add Facility</span>';
        showFacilities();
    }
}
...
function handleSearch() {
    ...
    } else if (currentSection === 'mountain') {
        showMountainViews(searchTerm);
    } else if (currentSection === 'facilities') {
        showFacilities(searchTerm);
    }
}
...
async function loadFacilities() {
    try {
        facilities = await api.get('/api/facilities');
...
function showFacilities(searchTerm) {
    var container = document.getElementById('facilitiesContainer');
    var notFound = document.getElementById('facilitiesNotFound');

    var list = facilities;

    if (searchTerm) {
        list = facilities.filter(function(facility) {
            var nameMatch = facility.name.toLowerCase().includes(searchTerm);
            var tagMatch = facility.tag && facility.tag.toLowerCase().includes(searchTerm);
        ...
        <div class="col-sm-6 col-lg-4 col-xl-3">
            <div class="mountain-card">
                <div class="mountain-img-wrapper">
                    ...
                    <div class="facility-tag-overlay">${facility.tag || 'FACILITY'}</div>
                    <button class="btn btn-danger btn-sm delete-btn" onclick="deleteFacility('${facility.facility_id}')">
                        <i class="bi bi-trash-fill"></i>
                    </button>
                </div>
                <div class="mountain-content">
                    <h6 class="mountain-title">${facility.name}</h6>
                    <p class="mountain-location" style="font-size: 0.8rem; color: #64748b;">
                        ${facility.description ? facility.description : 'No description'}
                    </p>
                </div>
            </div>
        </div>`;
...
async function saveFacility() {
    ...
    var tag = document.getElementById('facilityTag').value.trim();
    var description = document.getElementById('facilityDescription').value.trim();
    var image = document.getElementById('facilityImageBase64').value || 'https://images.unsplash.com/photo-1576013551627-0cc20b96c2a7?w=600';
    ...
    var nextNumber = facilities.length + 1;
    var facility_id = 'FC' + String(nextNumber).padStart(3, '0');
    ...
    try {
        ...
        var modal = bootstrap.Modal.getInstance(document.getElementById('addFacilityModal'));
        modal.hide();
...
function handleFacilityImageUpload(input) {
    ...
    var reader = new FileReader();
    reader.onload = function(e) {
        document.getElementById('facilityImageBase64').value = e.target.result;
        var preview = document.getElementById('facilityImagePreview');
        preview.src = e.target.result;
        preview.classList.add('show');
    };
    reader.readAsDataURL(file);
...
function resetFacilityForm() {
    ...
    document.getElementById('facilityTag').value = '';
    document.getElementById('facilityDescription').value = '';
    document.getElementById('facilityImageBase64').value = '';
    document.getElementById('facilityImagePreview').classList.remove('show');
}
